    ...
    for(let i = 0;i<requests.length;i++){
      if(requests[i]){ //确保不会是傻逼null
        if(requests[i].requestOpenid === event.requestOpenid){
          console.log("openid一样",event.requestOpenid);
        }
        if(requests[i].requestProjectId === event.requestProjectId){
          console.log("projectId一样",event.requestProjectId);
        }
        if(requests[i].requestTimeStamp === event.requestTimeStamp){
          console.log("requestTimeStamp一样",event.requestTimeStamp);
        }
        if(requests[i].requestOpenid === event.requestOpenid
        && requests[i].requestProjectId === event.requestProjectId
        && requests[i].requestTimeStamp === event.requestTimeStamp){
          console.log("执行了改变");
          requests[i].requestStatus = event.status === "agreed" ? "agreed" : "rejected"; //修改状态为新的状态
        }
      }
    }
    console.log("修改完之后的requests",requests);
    return db.collection("UserInfos").where({
      openid:OPENID
    }).update({
      data:{
